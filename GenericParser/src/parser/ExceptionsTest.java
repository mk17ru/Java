 ...
 */
public class ExceptionsTest extends parser.ParserTest {
    ...
    private void testParsingErrors() {
        for (final Op<String> op : parsingTest) {
            try {
                IntegerOperation s = new IntegerOperation(true);
                new ExpressionParser(s).parse(op.f);
    ...
    protected static void testOverflow() {
        ...
        testOverflow((a, b) -> a + b, "+", new Add(VX, VY, new IntegerOperation(true)));
        testOverflow((a, b) -> a - b, "-", new Subtract(VX, VY, new IntegerOperation(true)));
        testOverflow((a, b) -> a * b, "*", new Multiply(VX, VY, new IntegerOperation(true)));
        testOverflow((a, b) -> b == 0 ? Long.MAX_VALUE : a / b, "/", new Divide(VX, VY, new IntegerOperation(true)));
        testOverflow((a, b) -> -b, "<- ignore first argument, unary -", new Negate(VY, new IntegerOperation(true)));
    }
    ...
    protected TripleExpression parse(final String expression, final boolean reparse) {
        IntegerOperation s = new IntegerOperation(true);
        final Parser parser = new ExpressionParser<Integer>(s);
