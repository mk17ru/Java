...
public class BinarySearchMissing {
    ...
    // POST: R = "insertion point": eсли существует a[R] <= x && a[R - 1] > x иначе -R -1
    // && элементы массива a не изменяются
    public static int binaryIterativeSearch(int x, int[] a) {
        ...
        // r == "insertion point", a[r] <= x && a[r - 1] > x
        if (r == a.length || a[r] != x) {
    ...
    // Let be a[-1] == +inf && a[a.length] == -inf
    // PRE: (for i = -1..a.length : a[i] >= a[i + 1]) && (-1 <= l < r <= a.length)
